...
class TimeIt {
	constructor() {
		this.t = Date.now(); //new Date().getTime();
	}
	show(msg) {
		//shows ticks diff to last call of show
		let tNew = Date.now(); //new Date().getTime() - this.t;
		let tDiff = tNew - this.t;
		console.log(msg, tDiff, 'msecs');
		this.t = tNew;
	}
}
...
var UIDCounter = 0;
function getUID(pref = 'e') {
	UIDCounter += 1;
	return pref + UIDCounter;
}
var NAMED_UIDS = {};
...
function colorDarker(zero1, c, log = true) {
...
function colorLighter(zero1, c, log = true) {
	c = anyColorToStandardString(c);
	return pSBC(zero1, c, undefined, !log);
} //ok
...
function colorAreasRandomBlend(areaClass = 'area', mainDiv = 'mainDiv') {
	let c1 = randomColor();
	let c2 = randomColor();
	colorAreasBlend(c1, c2, areaClass, mainDiv);
}
function colorFromHue(h, s = 100, l = 50) {
	return {h: h, s: s, l: l};
}
function colorAreasOppositesBlend(hue = 120, areaClass = 'area', mainDiv = 'mainDiv') {
...
function colorAreasBlend(c1, c2, areaClass = 'area', mainDiv = 'mainDiv') {
	...
	let grid = document.getElementById(mainDiv);
...
function colorAreas_dep(hue0 = 260, nHues = 25, areaClass = 'area', mainDiv = 'mainDiv') {
...
function colorAreasN(hue0 = 120, nHues = 25, areaClass = 'area', mainDiv = 'mainDiv') {
	//console.log('colorAreas');
	//returns palettes[hue][darkness] (c1 first,darkest first)
...
function setCSSButtonColors(palset, ihue = 0) {
	...
	//backgrounds:
	root.style.setProperty('--bbg', pal[0]);
	root.style.setProperty('--bhbg', pal[2]);
...
function addGArea(gName, areaName = 'area_game', x = 0, y = 0, clearFirst = true) {
...
function addDivPos(dParent, x, y, w, h, {gap = 4, bg, fg, border, rounding, textAlign, fontSize}) {
...
function tableDimensions(w, h) {
	setCSSVariable('--w_2', '' + w + 'px');
	setCSSVariable('--h_2', '' + h + 'px');
	return {w: w, h: h};
}
...
function propDiffList(o_old, o_new, props) {
	...
	for (const prop of props) {
		...
		if (isdef(nval) && isdef(oval)) {
			...
			else if (typeof nval == 'object' && hasSameProps(nval, oval)) {
				continue;
...
function propDiffFast(o_old, o_new, deepCheck = false, optInOut = {}) {
...
function propDiffPlus(o_old, o_new, deepCheck = false, optInOut = {}) {
	//berechne diff in props
	let onlyOld = [];
	let onlyNew = [];
	let propChange = [];
...
function propDiffGSM(o_old, o_new, deepCheck = false, optInOut = {}) {
	//berechne diff in props
	let onlyOld = [];
	let onlyNew = [];
	let propChange = [];
